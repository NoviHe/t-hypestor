	...
	<div class="form_grup_with_judul">
		<?php
			$query = mysqli_query($koneksi, "SELECT max(id) as kodeTerbesar FROM db_media_images");
		...
		<?php 
			...
			{	
				...
				if ($size < 5048000 and ($type =='image/jpeg' or $type == 'image/png')) {
					...
					mysqli_query($koneksi, "insert into db_media_images (id,file,tipe,ukuran,tanggal) values ('$id','$name','$type','$size','$tanggal')");
		...
		<div class="row">	
			<?php
				$data = mysqli_query($koneksi,"select * from db_media_images");
